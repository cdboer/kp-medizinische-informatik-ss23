-- DROP TABLE mimiciv_derived.pre_septic_bg_2h; CREATE TABLE mimiciv_derived.pre_septic_bg_2h  AS
...
LEFT JOIN mimiciv_derived.bg bg
    ON ie.subject_id = bg.subject_id
        AND bg.charttime >= (CASE WHEN sepsis.sofa_time IS NULL THEN ie.intime ELSE sepsis.sofa_time - INTERVAL '4' HOUR END)
        AND bg.charttime <= (CASE WHEN sepsis.sofa_time IS NULL THEN ie.intime + INTERVAL '4' HOUR ELSE sepsis.sofa_time END)
