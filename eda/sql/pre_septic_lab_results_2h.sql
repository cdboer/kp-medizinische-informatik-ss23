-- DROP TABLE IF EXISTS mimiciv_derived.pre_septic_lab_2h; CREATE TABLE mimiciv_derived.pre_septic_lab_2h  AS
WITH 
...
, cbc AS (
    ...
    LEFT JOIN mimiciv_derived.complete_blood_count le
        ON le.subject_id = ie.subject_id
            AND le.charttime >= CASE WHEN sepsis.sofa_time IS NOT NULL THEN sepsis.sofa_time - INTERVAL '2' HOUR ELSE ie.intime END
            AND le.charttime <= CASE WHEN sepsis.sofa_time IS NOT NULL THEN sepsis.sofa_time ELSE ie.intime + INTERVAL '2' HOUR END
    GROUP BY ie.stay_id
)
