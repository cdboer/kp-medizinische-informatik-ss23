...
LEFT JOIN mimiciv_derived.vitalsign vs
    ...
    -- resolution could be improved ..
        AND vs.charttime >= (CASE WHEN sepsis.sofa_time IS NULL THEN ie.intime ELSE sepsis.sofa_time - INTERVAL '4' HOUR END)
        AND vs.charttime <= (CASE WHEN sepsis.sofa_time IS NULL THEN ie.intime + INTERVAL '4' HOUR ELSE sepsis.sofa_time END)
